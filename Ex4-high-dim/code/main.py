...
param_dict_default_FL = {
    'error_type': 'relative', # absolute/relative error compared to exact solution
    'd':10,
    'm_u':150, # width of solution network
    'm_v':150, # width of test function network
    'N_inside_train':45000, # sampling number on boundary
    'N_boundary_train':5000, # sampling number in domain
    'lr0_u':5e-5, # initial learning rate of solution optimizer
    'lr0_v':5e-4, # initial learning rate of test function optimizer
    'nu_u':20000, # adjust rate for lr_u
    'nu_v':20000,  # adjust rate for lr_v, for scheme, see paper.
...
def train(param_dict):
    ...
    # experiment params
    n_epoch = 100001 # number of outer iterations
    ...
    n_epoch_save_slu_plot = 500
    n_epoch_save_loss_plot = 5000

    flag_save_loss_plot = False
    ...
    lr_u_seq,lr_v_seq = generate_lr_scheme(n_epoch,lr0_u,lr0_v,nu_u,nu_v,alg=param_dict['alg'],restart_time=restart_time,pretrain_lr=pretrain_lr)
    ...
    norm_vec_batch = norm_vec(2*N_boundary_train,2)  # multiply by 2 since we will discard half of them
    ...
    while n_iter < n_epoch:
        if restart_time!=None:
            if n_iter == restart_time:
                ...
                net_u = generate_network(param_dict['net_u'],d,m_u,net_type='net_u',boundary_control_type = 'none',base_function = net_u) 
        ...
        for i in range(n_update_each_batch):
            x1_train = data_to_cuda(generate_uniform_points_in_cube(domain_parameter(2),N_inside_train,generate_type=sampling_type))
            x1_train_w = data_to_cuda(generate_uniform_points_in_cube(domain_parameter(d-2),N_inside_train,generate_type='npr'))
            x1_train = torch.cat((x1_train,x1_train_w),dim=1)

            if flag_diri_boundary_term == True:
                x2_train = data_to_cuda(generate_uniform_points_on_cube(domain_parameter(2),2*N_boundary_train//(2*2),generate_type=sampling_type)) # multiply by 2 since we will discard half of them
                x2_train = take_inflow_bd(x2_train,if_cuda=True)
                x2_train_w = data_to_cuda(generate_uniform_points_in_cube(domain_parameter(d-2),N_boundary_train,generate_type='npr'))
                x2_train = torch.cat((x2_train,x2_train_w),dim=1)
        ...
        if l2errorseq[n_iter] < bestl2error and n_iter % test_freq ==0:
        ...
        if maxerrorseq[n_iter] < bestmaxerror and n_iter % test_freq ==0:
        ...
        if n_iter%n_epoch_show_info==0 and verbose == True:
            ...
            #print(x_test)
            print('At T = %.2f, l2 error = %2.3e, maxerror = %2.3e\n'%(slice_time,l2error,maxerror), end='', flush=True)
    ...
    if flag_output_results == True:
        save_data_log(d,l2errorseq,maxerrorseq,PINN_lossseq,param_dict,n_epoch,N_inside_train,N_boundary_train,bestl2error,bestmaxerror)
...
if __name__ == '__main__':
    test_freq = 100  # the frenquency of evaluating the error

    pretrain_lr = (3e-4,3e-3)
    print(pretrain_lr)
    train(param_dict_default_FL)  

    # pretrain_lr = None
    # print(pretrain_lr) 
    # train(param_dict_default_PINN)  
