...
def do_visualize_slu(domain_intervals,net_u,net_v,true_solution,n_iter):
    ...
    plt.ylabel('$x_2$')
    plt.colorbar()
    plt.savefig("../result/image/slu_func_%d.png"%(n_iter))
    plt.close()
    ...
    plt.ylabel('$x_2$')
    plt.colorbar()
    plt.savefig("../result/image/phi_func_%d.png"%(n_iter))
    plt.close()
    return 0

# visualize the loss and save fig
def do_visualize_loss(l2errorseq,maxerrorseq,param_dict,n_iter):
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.set_title(param_dict, loc='center', wrap=True,fontsize=8)
