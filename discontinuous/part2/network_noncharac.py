...
class TanhResNet(nn.Module):  
    ...
    def forward(self, x):
        ...
        if self.boundary_control_type == 'none':
            ...
            x_1 = x[:,0]
            x_2 = x[:,1]
            mask = x_2 > (0.5*x_1-0.9)
            base = torch.zeros_like(x_1)
            base[~mask] = torch.exp(-5*(x_1[~mask]**2+(x_2[~mask]-0.9*x_1[~mask])**2))
...
# class ActanResNet(nn.Module):
#     def __init__(self, m):
#         super(ActanResNet, self).__init__()
...
#         self.outlayer = nn.Linear(m, 2, bias = False)
