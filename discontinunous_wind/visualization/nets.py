...
class ResNet_Tanh(nn.Module):  
    ...
    def forward(self, x):
        ...
        if self.boundary_control_type == 'none':
            ...
            return torch.sin(np.pi/4*x[:,1])*y.squeeze(1) * torch.sin(np.pi*r) + base

        elif self.boundary_control_type == 'net_v':
            return  torch.sin(np.pi/4*x[:,0])*y.squeeze(1)
    ...
    def forward(self, x):
        ...
        if self.base_function == None:
            ...
            if self.boundary_control_type == 'net_u':
                x_1 = x[:,0]
                x_2 = x[:,1]            
                return torch.sin(np.pi/4*x[:,1]) *y.squeeze(1) + self.base_function(x)
