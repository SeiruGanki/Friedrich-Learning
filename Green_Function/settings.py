...
def true_solution(x_batch):
    return g(np.exp(x_batch[:,0])-2/(d-1) * np.sum(x_batch[:,1:],axis=1)*np.exp(1/2),if_cuda=False)
...
def g_d(x_batch_diri): # the boundary function
    return g(torch.exp(x_batch_diri[:,0])-2/(d-1) * torch.sum(x_batch_diri[:,1:],axis=1)*np.exp(1/2),if_cuda=True)
...
def generate_matrix_A_batch(x_batch):
    ...
    A_batch[:,0,0] = np.exp(1/2)
    A_batch[:,0,1:] = torch.exp(x_batch[:,0]).view(N_1,1) * 1/2 * torch.ones([1,d-1])
    return A_batch
...
def compute_Aphi_prod_phi_norm_autograd(x_batch_inside,x_batch_diri,slu_net,phi_net,norm_vec_batch):
   ...
   # print('d@@@',d)
    ...
    part_2 = torch.dot(g_d(x_batch_diri),part_2_vec) /N_2 * d  # last 2 is the area/len
    part_4 = 0
