...
def generate_uniform_points_in_sphere(d,R,N,generate_type='npr'):
    if generate_type == 'lhs':
        F_points = pyDOE.lhs(d,N)
        points =  norm(loc=0, scale=1).ppf(F_points) 
    elif generate_type == 'npr':
        points = np.random.normal(size=(N,d))
    scales = ((np.random.uniform(0,R,(N,)))**(1/d)).reshape(N,1)*np.ones([N,d])
    r = np.sqrt(np.sum(points**2,axis=1)).reshape(N,1)*np.ones([N,d])
    return points/r * scales


# generate uniform distributed points on the boundary of domain {|x|<R}
def generate_uniform_points_on_sphere(d,R,N):
    if d == 1:
        points = np.array([[-R],[R]])    
        return points
    else:
...
def generate_lr_scheme(n_epoch,lr0_u,lr0_v,nu_u,nu_v,pretrain_lr=None,lower_bound_rate=0.01):
    ...
    pretrain_n_epoch = n_epoch //10
    
    if pretrain_lr != None:
        lr_u_seq_pre = np.zeros_like(lr_u_seq)
        lr_v_seq_pre = np.zeros_like(lr_v_seq)
        lr_u_seq_pre[:pretrain_n_epoch] = pretrain_lr[0]
        lr_u_seq_pre[pretrain_n_epoch:] = lr_u_seq[:-pretrain_n_epoch]
        lr_v_seq_pre[:pretrain_n_epoch] = pretrain_lr[1]
        lr_v_seq_pre[pretrain_n_epoch:] = lr_v_seq[:-pretrain_n_epoch]
...
def generate_network(net_name,dim,width,net_type='net_u',boundary_control_type='auto',base_function = None):
    if base_function == None:
        ...
        elif net_name == 'ResNet_Swish4':
            if boundary_control_type == 'auto':
                net = ResNet_Swish4(width,dim,boundary_control_type=net_type)  
            elif boundary_control_type == 'L2':
                net = ResNet_Swish4(width,dim,boundary_control_type='none')  
...
def generate_optimizer(net,optim_name,init_lr):
    if optim_name == 'Rmsprop':
        ...
        return torch.optim.Adam(net.parameters(),lr=init_lr,weight_decay=2e-5)
...
def do_evaluate_record_error(l2errorseq,maxerrorseq,net_u,true_solution,x_test,n_iter):
...
def do_visualize_slu(domain_intervals,domain_shape,net_u,net_v,true_solution,n_iter):
    d = domain_intervals.shape[0]
    if domain_shape == 'cube' or domain_shape == 'fan':
    ...
    elif domain_shape == 'ball':
        x_plot = np.arange(-domain_intervals[0,1], domain_intervals[0,1]+0.01, 0.015)
        y_plot = np.arange(-domain_intervals[1,1], domain_intervals[1,1]+0.01, 0.015)        
    ...
    plt.contourf(x,y,abs(z_1-z_2)/(abs(z_1)+abs(z_2)),cmap='RdYlGn', alpha = 0.8,levels = np.linspace(-1.5,1.5,20),vmin=-1,vmax=1)
    # plt.contourf(x,y,z_1,cmap='RdYlGn', alpha = 0.8,levels =20)
    plt.xlabel('$x_1$')
    plt.ylabel('$x_2$')
    plt.colorbar()
