...
def take_inflow_bd(x2_train,if_cuda=False):
    ...
    for k in range(d):
        x2_train_selected[k*N_2//(2*d):(k+1)*N_2//(2*d),:] = x2_train[(2*k)*N_2//(2*d):(2*k+1)*N_2//(2*d),:]
...
def generate_lr_scheme(n_epoch,lr0_u,lr0_v,nu_u,nu_v,alg='PINN',restart_time=None,pretrain_lr=None,lower_bound_rate=0.01):
    ...
    pretrain_niter = 5000
    if pretrain_lr != None:
        lr_u_seq_pre = np.zeros_like(lr_u_seq)
        lr_v_seq_pre = np.zeros_like(lr_v_seq)
        lr_u_seq_pre[:pretrain_niter] = pretrain_lr[0]
        lr_u_seq_pre[pretrain_niter:] = lr_u_seq[:-pretrain_niter]
        lr_v_seq_pre[:pretrain_niter] = pretrain_lr[1]
        lr_v_seq_pre[pretrain_niter:] = lr_v_seq[:-pretrain_niter]

        return lr_u_seq_pre,lr_v_seq_pre
    else:
        return lr_u_seq,lr_v_seq
...
def generate_network(net_name,dim,width,net_type='net_u',boundary_control_type='auto',base_function = None):
    if base_function == None:
        ...
        if net_name == 'ResNet_Swish':
            if boundary_control_type == 'auto':
                net = ResNet_Swish(width,dim,boundary_control_type=net_type)
            elif boundary_control_type == 'L2':
                net = ResNet_Swish(width,dim,boundary_control_type='none')
...
def do_evaluate_record_error(l2errorseq,maxerrorseq,net_u,true_solution,x_test,n_iter,error_type='absolute'):
...
def do_evaluate_error_at_slice(T,x_test,net_u,true_solution,error_type='absolute'):
    x_slice_test = np.ones_like(x_test) * T
    x_slice_test[:,1:] = x_test[:,1:]
    if error_type == 'relative':
        l2error = evaluate_rel_l2_error(net_u,true_solution, x_slice_test)
        maxerror = evaluate_rel_max_error(net_u, true_solution,x_slice_test)
    elif error_type == 'absolute':
        l2error = evaluate_l2_error(net_u,true_solution, x_slice_test)
        maxerror = evaluate_max_error(net_u, true_solution,x_slice_test)
...
def compute_net_value_on_plots(x,y,d,net,net_like=True,find_slide=None):  # find_slide argument takes the form (a,b), a means the value of a-th dim is fixed to be b.
    size = list(x.shape)[0]
    points = np.stack((x,y),axis=2).reshape(size*size,2)
    if d>2:
        point_batch = np.ones([size*size,d]) * 1/2
        point_batch[:,:2] = points
    if find_slide!= None:
...
def do_visualize_slu(domain_intervals,net_u,net_v,true_solution,n_iter):
    ...
    plt.contourf(x,y,z_1-z_2,cmap='RdYlGn', alpha = 0.8,levels = 20)
    plt.xlabel('$t$')
    plt.ylabel('$x_1$')
    plt.colorbar()
...
# log
def save_data_log(d,l2errorseq,maxerrorseq,lossseq,param_dict,n_epoch,N_inside_train,N_boundary_train,bestl2error,bestmaxerror):
    localtime = time.localtime(time.time())
    time_text = str(localtime.tm_mon)+'_'+str(localtime.tm_mday)+'_'+str(localtime.tm_hour)+'_'+str(localtime.tm_min)
    ...
    text = text + 'error_type = ' + str(param_dict['error_type']) + ', '+'\n'    
    text = text + 'min l2 error = ' + str(min(bestl2error)) + ', '
    text = text + 'min max error = ' + str(min(bestmaxerror)) + ', '
