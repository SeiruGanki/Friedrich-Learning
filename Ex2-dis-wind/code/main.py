...
param_dict_default_PINN_L2 = {
    ...
    'optimizer_u':'Adam', # optimizer for slu net Adam/Rms
    'optimizer_v':'Rmsprop', # optimizer for test net Adam/Rms
    'boundary_control_type':'L2' # ways of PINN handling the boundary, default autometic, can be L2 
}

param_dict_default_PINN_auto = {
    'error_type': 'relative', # absolute/relative error compared to exact solution
    'm_u':150, # width of solution network
    'm_v':150, # width of test function network
    'N_inside_train':45000, # sampling number on boundary
...
def train(param_dict):
    ...
    x_test = generate_uniform_points_in_fan(domain_intervals,N_test,0,np.pi)
    ...
    while n_iter < n_epoch:
        ...
        if alg == 'Friedrich':
            for i in range(n_update_each_batch_test):
                ...
                if flag_diri_boundary_term == True:
                    x2_train = data_to_cuda(generate_uniform_points_on_fan(domain_intervals,N_boundary_train,0,np.pi,direction='outflow'))     
        ...
        l2errorseq,maxerrorseq = do_evaluate_record_error(l2errorseq,maxerrorseq,net_u,true_solution,x_test,n_iter,error_type = param_dict['error_type'])
...
if __name__ == '__main__':
    ...
    train(param_dict_default_PINN_auto)
    # train(param_dict_default_PINN_L2)
