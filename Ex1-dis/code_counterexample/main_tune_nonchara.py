...
def train():
    ...
    print('Train start! lr_u = %.2e, exp_bench_u = %d'%(lr_u,exp_bench_u))
    ...
    while k < n_epoch:
        ...
        for i in range(n_update_each_batch):
            ...
            ## Compute the loss  
            loss_1 = compute_least_square_loss(x1_batch,u_net)
        ...
        if l2errorseq[k] < bestl2error:
            state = {'u_net': u_net,'besterror':l2errorseq[k]}
        ...
        if k%n_epoch_show_info==0:
            print("epoch = %d" %(k))
            print(", loss1 =",loss_1.item(), end='', flush=True)
            print('')
