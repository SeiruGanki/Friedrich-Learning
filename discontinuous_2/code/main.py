...
def train(param_dict):
    ...
    if alg == 'Friedrich' and restart_time != None:
        net_u = generate_network(param_dict['net_u'],d,m_u//2,boundary_control_type='net_u')
    else:
    ...
    while n_iter < n_epoch:
        ...
        for i in range(n_update_each_batch):
            x1_train = data_to_cuda(generate_uniform_points_in_cube(domain_intervals,N_test,generate_type=sampling_type))
...
if __name__ == '__main__':
    fspace = {
        ...
        # 'lr0_u':3e-4 * (1 + hp.randint('lr0_u',10)),
        'lr0_u': hp.choice('lr0_u', [1e-4]),
        # 'lr0_v':3e-3 * (1 + hp.randint('lr0_v',10)),
        'lr0_v': hp.choice('lr0_v', [1e-3]),

        'nu_u':5000 * (1 + hp.randint('nu_u',4)),
        #'nu_u':hp.choice('nu_u', [5000]),
        'nu_v':5000 * (2 + hp.randint('nu_v',3)),
