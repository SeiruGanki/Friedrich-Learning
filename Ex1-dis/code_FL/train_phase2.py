...
# from network_noncharac import ReLUResNet_base
...
def train2(lr_u,lr_v,exp_bench_u,exp_bench_v,pretrain_num):
    ########### basic parameters #############
    ...
    # lr_u = 3e-4  # actually at epoch 1000
    # lr_v = 3e-3
    # exp_bench_u = 10000   # after epoch 1000
    # exp_bench_v = 10000

    domain_shape = 'cube' ## the shape of domain 
    domain_intervals = domain_parameter(d)
    loss_type = 'frac'
    flag_adjust_decaying = True
    ...
    n_epoch_show_info = 100 # max([round(n_epoch/100),1]) # how many epoch will it show information
    ...
    load_data = torch.load('./checkpoint/base.t7')
    u_base_net = load_data['u_net']   
    v_net = load_data['v_net']
    print('base',load_data)

    for param in u_base_net.named_parameters():
        param[1].requires_grad = False
    
    torch.manual_seed(0)
    np.random.seed(0)    

    u_net = ReLUResNet_base(m_u,dim=d,boundary_control_type = 'u_net',base_function=u_base_net) #这个网络是原函数u的模型网络

    #--------------------------------------------- 
    ########## the fairness #############
    for param in u_net.named_parameters():
    ...
    if flag_output_results == True:
        ...
        torch.save(state, './checkpoint/best_%.4f.t7'%(bestl2error))
