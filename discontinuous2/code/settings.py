...
non_singular = True # please check nets.py
...
def compute_Aphi_prod_phi_norm_autograd(x_batch_inside,x_batch_diri,slu_net,phi_net,norm_vec_batch):
    ...
    part_1 = - torch.dot(A_mul_grad,slu_net_output_inside)/ N_1 * 1
    
    # compute tidle T norm
    A_mul_grad_norm = torch.sum((-A_mul_grad+c*phi_net_output_inside)** 2) * 1
    ...
    part_2 = torch.dot(g_d(x_batch_diri),part_2_vec) /N_2 * 2  # last 2 is the area/len

    part_4 = c * torch.dot(slu_net_output_inside,phi_net_output_inside) / N_1 * 1

    part_5 = - torch.dot(f(x_batch_inside),phi_net_output_inside) / N_1 * 1
