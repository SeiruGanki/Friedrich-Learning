...
def train(param_dict):
    ...
    N_inside_train = 40000 # number of training points inside domain
    N_boundary_train = 1000 # number of training points on boundary
    ...
    net_v = generate_network(param_dict['net_v'],d,m_v,net_type = 'net_v',boundary_control_type=param_dict['boundary_control_type'])
    ...
    while n_iter < n_epoch:
        ...
        for i in range(n_update_each_batch):
            ...
            if flag_diri_boundary_term == True:
                x2_train = data_to_cuda(generate_uniform_points_on_cube(domain_intervals,2*N_boundary_train//4,generate_type=sampling_type)) # multiply by 2 since we will discard half of them
        ...
        if alg == 'Friedrich':
            for i in range(n_update_each_batch_test):
                ...
                if flag_diri_boundary_term == True:
                    x2_train = data_to_cuda(generate_uniform_points_on_cube(domain_intervals,2*N_boundary_train//4,generate_type=sampling_type))     
                    x2_train = take_inflow_bd(x2_train,if_cuda=True)
...
if __name__ == '__main__':
    fspace = {
        'm_u': hp.choice('m_u', [100]),
        'm_v': hp.choice('m_v', [100]),

        #'lr0_u':2e-4 * (2 + hp.randint('lr0_u',5)),
        'lr0_u': hp.choice('lr0_u', [8e-4]),
        #'lr0_v':2e-4 * (4 + hp.randint('lr0_v',8)),
        'lr0_v': hp.choice('lr0_v', [1.4e-3]),

        # 'nu_u':4000 * (2+ hp.randint('nu_u',2)),
        'nu_u':hp.choice('nu_u', [10000]),
        ...
        'boundary_control_type':hp.choice('boundary_control_type', ['L2','auto'])
        }
