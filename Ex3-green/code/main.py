...
param_dict_default_FL = {
    ...
    'lr0_u':1.5e-5, # initial learning rate of solution optimizer
    'lr0_v':3e-5, # initial learning rate of test function optimizer
    'nu_u':10000, # adjust rate for lr_u
    'nu_v':10000,  # adjust rate for lr_v, for scheme, see paper.
...
def train(param_dict):
    ...
    # domains
    domain_shape = 'ball' # the shape of domain  
    ...
    # optionals 
    restart_time = param_dict['restart_time']  # restart method is deactivated this case
    ...
    lr_u_seq,lr_v_seq = generate_lr_scheme(n_epoch,lr0_u,lr0_v,nu_u,nu_v,pretrain_lr=pretrain_lr)
    ...
    x_test = generate_uniform_points_in_sphere(d,domain_intervals[0,1],N_test,generate_type=param_dict['sampling_type'])
    ...
    while n_iter < n_epoch:
        ...
        for i in range(n_update_each_batch):
            x1_train = data_to_cuda(generate_uniform_points_in_sphere(d,domain_intervals[0,1],N_inside_train,generate_type=param_dict['sampling_type']))

            if flag_diri_boundary_term == True:
                x2_train = data_to_cuda(generate_uniform_points_on_sphere(d,domain_intervals[0,1],N_boundary_train))
                norm_vec_batch = norm_vec(N_boundary_train,d,x2_train)
        ...
        if l2errorseq[n_iter] < bestl2error and n_iter % test_time == 0:
        ...
        if maxerrorseq[n_iter] < bestmaxerror and n_iter % test_time == 0:
            bestmaxerror = maxerrorseq[n_iter]  

        if flag_save_slu_plot == True and n_iter % n_epoch_save_slu_plot == 0:
            do_visualize_slu(domain_intervals,domain_shape,net_u,net_v,true_solution,n_iter)
